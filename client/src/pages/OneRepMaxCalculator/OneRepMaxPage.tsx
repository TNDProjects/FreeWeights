...
import { ConvertToPounds } from "../../utils/units/units.ts"
import type { Max } from "../OneRepMaxCalculator/types/types.ts";
import type { WeightUnit } from "../OneRepMaxCalculator/types/types.ts";
...
const OneRepMaxPage = () => {
  ...
  const [weightUnitEntered, setWeightUnitEntered] = useState<WeightUnit>("lbs"); //current weight unit in select input
  const [weightUnitSaved, setWeightUnitSaved] = useState<WeightUnit>("lbs"); //current saved weight unit after hitting calculate button
  ...
  const calculateOneRepMax = () => {
    ...
    if (weightUnitEntered == 'lbs') {
      maxInPounds = calculateMax(actualWeight, actualReps);
      maxInKG = ConvertToKilograms(maxInPounds);

    } else {
      maxInKG = calculateMax(actualWeight, actualReps);
      maxInPounds = ConvertToPounds(maxInKG);
    }
  ...
  const displayOneRepMax = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setWeightUnitSaved(weightUnitEntered);
    calculateOneRepMax();
  };
  ...
  return (
    <div className="w-full max-w-3xl">
      ...
      <EnterLift
        ...
        weightUnit={weightUnitEntered}
        onWeightUnitChange={setWeightUnitEntered}
        buttonText="calculate"
        onSubmit={displayOneRepMax}
      />
 
      {oneRepMaxInPounds !== null && (
        <div className="animate-up pt-12">
          <div className="border-2 border-dark dark:border-light rounded-xl flex flex-col">
            <div className="rounded p-6 text-center">
              <p className="text-sm md:text-lg pb-4 tracking-tighter">
                your estimated one-rep max is:
              </p>
              <p className="text-4xl md:text-5xl lg:text-6xl tracking-tight">
                {weightUnitSaved === "lbs" ? (
                  <>
                    <span className="font-bold">{oneRepMaxInPounds}<span className="text-2xl">lbs</span>{" "}</span>
                    <span className="font-normal text-grey">/ {oneRepMaxInKg}<span className="text-2xl">kg</span></span>
                  </>
                ) : (
                  <>
                    <span className="font-bold">{oneRepMaxInKg}<span className="text-2xl">kg</span>{" "}</span>
                    <span className="font-normal text-grey">/ {oneRepMaxInPounds}<span className="text-2xl">lbs</span> </span>
                  </>
                )}
              </p>
            </div>
